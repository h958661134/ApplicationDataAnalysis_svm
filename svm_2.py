...
with open("训练数据集1.csv") as f:
    with open("c_测试结果1_1.csv","w",newline='') as f1:
        f_csv = csv.DictReader(f)
        ...
        for row in f_csv:
            ...
            list = []
            list.append(row['click_time'][row['click_time'].find(" ")+1:row['click_time'].find(":")]+row['click_time'][row['click_time'].find(":")+1:])
